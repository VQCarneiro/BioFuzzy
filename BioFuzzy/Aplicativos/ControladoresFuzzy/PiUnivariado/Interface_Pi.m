...
% Last Modified by GUIDE v2.5 09-Aug-2017 00:01:42
...
PMFAV = get(handles.popupmenu3,'String');
PMFAV = PMFAV{posSel};
PMFAV = str2num(PMFAV);
handles.PMFAV = PMFAV;
...
PMDESF = get(handles.popupmenu4,'String');
...
a = bar(x,y);
xlabel('Genótipos','fontsize',12,'fontweight','b');ylabel('Pertinências','fontsize',12,'fontweight','b');
hleg2 = legend('Adaptabilidade Geral','Pouco Adaptado', 'Adaptabilidade a Ambientes Favoráveis','Adaptabilidade a Ambientes Desfavoráveis');
set(hleg2,'Location','NorthWest','Fontsize', 10,'FontAngle','italic')
set(gca,'Fontsize', 12,'FontWeight','b');ylim([0 1.4]);xlim([0 17]);
colormap cool
y2 = 0.5*(ones(1,nll))
...
nomeExp = [pathname, nomeWord];
handles.nomeExp = nomeExp;
...
pathname = 'C:\BioFuzzy\Exemplos\piUnivariado\';
handles.pathname = pathname;
filename = 'dados.txt';
handles.filename = filename;
arquivo = 'C:\BioFuzzy\Exemplos\piUnivariado\dados.txt';
handles.arquivo = arquivo;
set(handles.edit3,'String',arquivo);
set(handles.edit3,'ForegroundColor','Black');
