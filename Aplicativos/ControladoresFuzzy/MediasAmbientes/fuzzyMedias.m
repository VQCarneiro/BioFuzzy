...
%% Entradas
%  [dadosOriginais texto comp] = xlsread('mga.xls');
% dadosOriginais = load('dadosCE.txt');
% tipo = 1;
%dadosOriginais = dadosOriginais(:,2:end);
function [matrizResumo tabelaClassOriginal tabelaClassPadronizado pertinencias parametros] = fuzzyMedias(dadosOriginais,PM,tipo)
dadosOriginais = dadosOriginais(:,2:end);
...
for i = 1:nVariaveis
    ...
    a.input(i).mf(1).params = [Parametro1 Parametro2];
    a.input(i).mf(2).name = 'Média Alta';
    a.input(i).mf(2).type = 'smf';
    a.input(i).mf(2).params = [Parametro1 Parametro2];
end

% figure('Name','Entradas');
% fator = round(nVariaveis/2)
% for i = 1:nVariaveis
%     
% subplot(2,fator,i)
% plotmf(a,'input',i)
